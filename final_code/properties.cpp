...
int properties_t::read_file(const char* filename)
{
  ...
  while(std::getline(in,line))
  {
    ...
    else if(line.substr(0,8) == "include " || line.substr(0,9) == "-include ")
    {
      ...
      if(newfile.length())
      {
        ...
        if(newfile[0] != '/' && (pos = oldfile.rfind('/')) != std::string::npos)
          newpath = oldfile.substr(0,pos+1) + newfile;
        if(read_file(newpath.c_str()) && line[0] != '-')
          error("Could not open properties file %s!\n",newfile.c_str());
      }
    }
    else if(line != "")
      warn("Properties file contains unknown command %s!",line.c_str());
  }
